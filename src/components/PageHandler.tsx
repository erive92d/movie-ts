...
export default function PageHandler({handlePage, page}:pageProps) {
  ...
  return (
    <div className="join grid  grid-cols-2 lg:w-1/3 lg:mx-auto">
        <button name="back"  onClick={handlePage} className={`${page === 1 ? "btn-disabled " : ""}join-item btn-sm btn btn-outline text-white`}>Previous page</button>
        <button name="next" onClick={handlePage} className="join-item btn btn-sm btn-outline text-white">Next</button>
    </div>
